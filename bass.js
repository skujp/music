...
var DEF_INST = "piano";
var DEF_SUPPORT_INST = new Set([DEF_INST, "organ", "acoustic", "edm"]);
var instrument;                                                     
...
URL.createObjectURL = function(blob) {
    ...
    soundBlobUrls.push(url);
    if (soundBlobUrls.length > MEMORY_THRESHOLD) {
        const oldestUrl = soundBlobUrls.shift();             
        URL.revokeObjectURL(oldestUrl);
        error.msg = `Threshold reached. Auto-freed occupied memory: ${oldestUrl}`;
        Synth.clearCache();    
    }
    return url;
};
try {
    Synth.clearCache();  
    instrument = Synth.createInstrument(DEF_INST);      
} catch (exception) {
    if (exception instanceof TypeError) {
        const problem = `Wrong version of audiosynth.js. Please download again from the bass repository.`;
        error.msg = problem;
        oops(problem, 'error');
        return;
    } else if (exception instanceof ReferenceError) {
        const problem = `audiosynth.js is not loaded. BASS can process the sheet music but playback is not available.`;
        error.msg = problem;
        oops(problem, 'error');
    } else {
        const problem = `Unidentified exception: ${exception}`;
        error.msg = problem;
        oops(problem, 'warning');
    }
}
bind(TheBass, {setFullAsyncDebug, getFullAsyncDebugMode, oops, 
                   playChord, playSequencer, stopSequencer, 
...
var SMP_TESTCASE = "Title:Rhythmeus\nComposer:Chordius\nPerformer:Melodius\n\n3/4 |: F / F7 | % | [1. G D G/D :| [2. Bmaj7 Aaug Fdim ||";      
...
function help() {
    ...
    manual += "" + "\n";
    manual += "14. Pulse /" + "\n";
    manual += "A pulse / means the user can either play the previous note or sustain it." + "\n";
    manual += "By default, the BASS system will play the previous note (staccato style)." + "\n";
    ...
    manual += "Users can save to .bass file and send it to the music hobbyist's email: bassthemusic101 |at| gmail |dot| com" + "\n";
    ...
    manual += "" + "\n";
    manual += "The BASS system is written by a music enthusiast to help you learn the basic rhythms, " + "\n"; 
    manual += "chords (staccato), and very basic compositions. It is by no means to replace a " + "\n";
...
function _playSequencerChord(sequencerChord = BM, octave = DEF_OCTAVE, duration = DEF_DURATION) {
    if (sequencerChord != BM) {
        ...
        if (sequencerChord[1]) {
            instrument.play(sequencerChord[1], octave - 1, duration);
        }
        sequencerChord[0].forEach(note => {
            instrument.play(note, octave, duration); 
        });
    } else {  
        Synth.setVolume(MIN_VOL);
        instrument.play(FAKE, MIN_OCTAVE, duration); 
        Synth.setVolume(MAX_VOL);
    }
}
...
async function playSequencer(sequencerObj) {
  ...
  try {
    Synth.setVolume(MIN_VOL);
    _playSequencerChord(sequencer[0], MIN_OCTAVE, DEF_DURATION);  
    Synth.setVolume(MAX_VOL);
    ...
    while (!signal.aborted) { 
      ...
      while (i < n) {
        ...
        _playSequencerChord(sequencer[i], DEF_OCTAVE, DEF_DURATION); 
        await _delay(DEF_MSEC, signal); 
...
function setInstrument(inst) {
    ...
    try {
        instrument = Synth.createInstrument(inst);      
    } catch (exp) {
        error.msg = `[EXCEPTION] in Synth.createInstrument for ${inst}`;
        return error;
    }
}
